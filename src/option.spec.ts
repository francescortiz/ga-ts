...
describe("Option", () => {
    ...
    it("Option.resultMap evaluates to Result", () => {
        const option = -1 > 0 ? Some(2) : None;
        const result: Result<number, NoValueError> = option.resultMap((value) => value + 2);

        expect(result).toEqual(Err(new NoValueError()));
    });

    it("None.resultMap evaluates to Err with an ASYNC callback", () => {
        const option = None;
        const result: Err<NoValueError> = option.resultMap((value: number) =>
    ...
    it("Some.attemptMap evaluates to Err with a callback that throws", () => {
        const option = Some(1);
    ...
    it("None.attemptMap evaluates to Err with an ASYNC callback", () => {
        const option = None;

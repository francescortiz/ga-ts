...
export type Ok<T> = {
    ...
    mapError<R>(f: MapFn<unknown, R>): R extends Promise<Any> ? AsyncOk<T> : Ok<T>;
    flatMap<F extends FlatMapFn<T, unknown, Any, Any>>(
        f: F,
...
export type Err<E> = {
    ...
    value: never;
    error: E;
    map(f: MapFn<unknown, unknown>): Err<E>;
    mapError<R>(f: MapFn<E, R>): R extends Promise<infer R2> ? AsyncErr<R2> : Err<R>;
    flatMap(f: FlatMapFn<unknown, unknown, Any, Any>): Err<E>;
    attemptMap(f: MapFn<unknown, unknown>): Err<E>;
};
...
export type AsyncErr<E> = {
    value: Promise<never>;
    error: Promise<E>;
    map(f: unknown): AsyncErr<E>;
    mapError<R>(f: MapFn<E, R>): R extends Promise<infer R2> ? AsyncErr<R2> : AsyncErr<R>;
    flatMap(f: unknown): AsyncErr<E>;
    attemptMap(f: unknown): AsyncErr<E>;
} & Promise<Err<E>>;
